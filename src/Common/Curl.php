...
class Curl {
     ...
     * @GET请求
     */
    public function Get($url, array $params = array(), $header) {
        if ($params) {
            if (strpos($url, '?')) {
     ...
     * @POST请求
     */
    public function Post($url, $params = array(),$par_type=null) {
        $this->Start($url);
        curl_setopt($this->ch_, CURLOPT_SSL_VERIFYPEER, 0);
        curl_setopt($this->ch_, CURLOPT_POST, true);
        //发送json数据
        if($par_type=='json'){
            curl_setopt($this->ch_, CURLOPT_POSTFIELDS, json_encode($params));//json的字符串
            curl_setopt($this->ch_, CURLOPT_TIMEOUT, 120);
            curl_setopt($this->ch_, CURLOPT_RETURNTRANSFER, true);
            curl_setopt($this->ch_, CURLOPT_HTTPHEADER, array('Content-Type: application/json;charset=utf-8','Content-Length:'.strlen(json_encode($params))));
        }elseif($par_type=='post_img'){
            $cfile = curl_file_create($params['img_path'],'image/pjpeg',"C:\\".$params['bz'].".JPG");
            $data = array($params['img_name']=> $cfile);
            unset($params['img_path']);
            $data=array_merge($data,$params);
            curl_setopt($this->ch_, CURLOPT_POST,1);
            curl_setopt($this->ch_, CURLOPT_POSTFIELDS, $data);
            curl_setopt($this->ch_, CURLOPT_HTTPHEADER, array('Content-Type: multipart/form-data;charset=utf-8'));
            $this->body_ = $this->Execx();
            $result=$this->Close($this->body_);
            return $result;
        }else{
            curl_setopt($this->ch_, CURLOPT_TIMEOUT, Curl::time_);
